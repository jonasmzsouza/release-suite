...
export function syncTags(cwd = process.cwd()) {
  try {
    // Search for remote tags without deleting local ones
    execSync("git fetch --tags --force", { cwd, stdio: "ignore" });
  } catch {
    // silent by design (doesn't break the flow)
  }
}

export function getAllTagsSorted(cwd = process.cwd()) {
  syncTags(cwd);
...
export function getCommits(range, cwd) {
  try {
    return run(`git log ${range} --pretty=format:%H%x1f%s%x1f%b`, cwd).split("\n").filter(Boolean);
  } catch {
    return [];
  }
}
...
export function parseCommit(line) {
  ...
  const prMatch = cleanSubject.match(/\(#(\d+)\)/) || cleanSubject.match(/#(\d+)/);
