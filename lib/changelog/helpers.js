...
export function renderChangelog(version, buckets) {
  ...
  pushSection(
    sections,
    emojis ? "💥 Breaking Changes" : "Breaking Changes",
    buckets.breaking,
    provider
  );
  pushSection(sections, emojis ? "✨ Features" : "Features", buckets.feat, provider);
  pushSection(sections, emojis ? "🐛 Fixes" : "Fixes", buckets.fix, provider);
  pushSection(sections, emojis ? "⚡ Performance" : "Performance", buckets.perf, provider);
  pushSection(sections, emojis ? "♻ Refactors" : "Refactors", buckets.refactor, provider);
  pushSection(sections, emojis ? "📚 Documentation" : "Documentation", buckets.docs, provider);
  pushSection(sections, emojis ? "🧪 Tests" : "Tests", buckets.test, provider);
  pushSection(sections, emojis ? "🔧 Chores" : "Chores", buckets.chore, provider);
  pushSection(sections, emojis ? "🤖 CI" : "CI", buckets.ci, provider);
  pushSection(sections, emojis ? "📦 Other" : "Other", buckets.other, provider);
...
export function insertReleaseSection(existing, newSection) {
  ...
  if (!existing.startsWith("# Changelog")) {
    throw new Error("Invalid CHANGELOG.md format: missing '# Changelog' header");
  }

  const lines = existing.split("\n");
  const firstReleaseIndex = lines.findIndex((l) => l.startsWith("## "));
...
export function normalizeLegacyChangelog(existing) {
  ...
  throw new Error("Invalid CHANGELOG.md format: unable to normalize legacy file");
}
...
export function extractChangelogHeader(existing = "") {
  ...
  return lines.slice(0, firstReleaseIndex).join("\n").trimEnd() + "\n\n";
}
...
function stripInlineRefs(subject = "") {
  return subject
    .replace(/\((?:#\d+(?:,\s*)?)+\)/g, "")
    .replace(/\s+/g, " ")
    .trim();
 ...
 */
function pushSection(target, title, commits = [], provider) {
  if (!commits.length) return;

  target.push(`### ${title}`);
  for (const commit of commits) {
    const subject = stripInlineRefs(commit.subject);
    const refs = commit.references ?? [];
    const links = refs.map((r) => `[#${r.id}](${provider.buildUrl(r)})`).join(", ");
    const line = links ? `- ${subject} (${links})` : `- ${subject}`;
...
export function hasAnyContent(buckets) {
  return Object.values(buckets).some((list) => Array.isArray(list) && list.length > 0);
}
