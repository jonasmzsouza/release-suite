...
import { getLastTag, getCommits, parseCommit } from "../git/git.js";
 ...
 * }
 */
export function generateChangelog({ cwd = process.cwd(), dryRun = false } = {}) {
  ...
  const targetPath = path.join(cwd, dryRun ? "CHANGELOG.dry-run.md" : "CHANGELOG.md");

  let existing = fs.existsSync(sourcePath) ? fs.readFileSync(sourcePath, "utf8") : "";
