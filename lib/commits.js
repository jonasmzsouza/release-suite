 ...
 */
/**
 * Normalize an array of commit objects into a flattened list of commits with cleaned subjects.
 *
 * For each commit in the input array:
 * - Skips commits that do not pass isValidCommit(commit).
 * - Splits commit.subject by newline, trims each line and removes empty lines.
 * - Strips common bullet markers ('-' or '*') at the start of each line.
 * - Normalizes each resulting line via normalizeSubject(line).
 * - Skips any lines that normalize to a falsy value.
 * - For each remaining normalized subject line, pushes a shallow copy of the original commit
 *   with the subject replaced by the normalized subject.
 *
 * The original commit objects are not mutated; the returned array contains new objects
 * that spread the original commit properties and override the subject property.
 *
 * Note: This function relies on external helpers isValidCommit and normalizeSubject.
 *
 * @param {Array<Object>} [commits=[]] - Array of commit objects. Each commit is expected to
 *   have at least a `subject` string property.
 * @returns {Array<Object>} A new array of commit objects with cleaned/normalized `subject`
 *   values. Commits or subject lines that are invalid or normalize to falsy values are omitted.
 *
 * @example
 * // Given commits like:
 * // [{ sha: 'abc', subject: '- feat: add foo\n- fix: bar' }, ...]
 * // normalizeCommits(commits) =>
 * // [{ sha: 'abc', subject: 'feat: add foo' }, { sha: 'abc', subject: 'fix: bar' }, ...]
 *
 * @see isValidCommit
