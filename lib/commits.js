...
function classifyReferences(subject = "", body = "") {
  const text = `${subject}\n${body}`;

  const prFromMerge = text.match(/pull request #(\d+)/i)?.[1];
  ...
  while ((match = regex.exec(text))) {
    ...
    refs.push({
      id,
      type: prFromMerge && Number(prFromMerge) === id ? "pr" : "issue"
    });
  }

  // removes duplicates while keeping the “stronger” type (pr > issue)
  const map = new Map();

  for (const ref of refs) {
    if (!map.has(ref.id) || ref.type === "pr") {
      map.set(ref.id, ref);
    }
  }
  ...
  for (const commit of commits) {
    ...
    for (const line of lines) {
      ...
      const links = refs.map((ref) => ({
        ...ref,
        url: provider ? provider.buildUrl(ref) : null
      }));
