...
export function isMergeCommit(commit) {
  return /^merge\b/i.test(commit.subject) || /^merge pull request\b/i.test(commit.subject);
}
...
export function createReferenceClassifier() {
  const repo = getRepoInfo();
  const provider = repo?.provider || "generic";

  return function classifyReferences(subject = "", body = "") {
    const text = `${subject}\n${body}`;
    ...
    const addRef = (id, type) => {
      const existing = refs.get(id);

      // prefer PR over issue
      if (!existing || type === "pr") {
        refs.set(id, { id, type });
      }
    };

    // --- #123 notation (GitHub / GitLab / Bitbucket issues)
    while ((match = issueRegex.exec(text))) {
      ...
      if (provider === "github") {
        if (
          new RegExp(`pull request #${id}`, "i").test(text) ||
          new RegExp(`\\(#${id}\\)`).test(subject) ||
          new RegExp(`PR\\s*#${id}`, "i").test(text)
        ) {
          type = "pr";
        }
      }
      ...
      if (provider === "gitlab") {
        if (
          new RegExp(`merge request !${id}`, "i").test(text) ||
          new RegExp(`\\(#${id}\\)`).test(subject)
        ) {
          type = "pr";
        }
    ...
    // --- GitLab MR notation (!123)
    if (provider === "gitlab") {
      while ((match = mrRegex.exec(text))) {
        const id = Number(match[1]);
        addRef(id, "pr");
      }
    }

    // --- closing keywords → always issue
    const closingRegex = /(close[sd]?|fix(e[sd])?|resolve[sd]?)\s+#(\d+)/gi;

    while ((match = closingRegex.exec(text))) {
      const id = Number(match[3]);
      addRef(id, "issue");
    }

    return [...refs.values()].sort((a, b) => a.id - b.id);
  };
}
...
export function normalizeCommits(commits = []) {
  ...
  const classifyReferences = createReferenceClassifier();
  ...
  for (const commit of commits) {
    ...
    for (const line of lines) {
      ...
      const refs = classifyReferences(subject, commit.body);
...
export function categorizeCommits(commits = []) {
  ...
  for (const commit of commits) {
    ...
    const clearedSubject = cleanSubject(rawSubject);
